...
import com.immersion.neuro.viewmodel.BleViewModel
import com.immersion.neuro.presentation.adapters.ScanAdapter
...
class MainActivity : AppCompatActivity(), ScanAdapter.ScanItemClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.getScanResults().observe(this, Observer {
            ...
            it?.let {
                viewModel.getHeartBatteryInfo().observe(this, {
        ...
        viewModel.getConnectionStatus().observe(this, Observer { it ->
            it?.let {
                if (it.connectionStatus is ConnectionStatus.NotEngaged) {
                    viewModel.getScanResults().value?.get(it.device.address)?.connectionStatus =
                        it.connectionStatus
                    scanAdapter.submitList(
                        viewModel.getScanResults().value?.values?.toList() ?: emptyList()
                    )
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        when (requestCode) {
            ...
            else Toast.makeText(this, R.string.ble_permission_not_granted, Toast.LENGTH_SHORT)
                .show()
        }
